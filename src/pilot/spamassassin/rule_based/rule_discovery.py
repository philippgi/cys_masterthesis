"""
Discovers SpamAssassin rules for the rule-based pilot study.

Rule definitions are extracted from the SpamAssassin container, classified
by message surface and estimated sensitivity to U+200B salting, and exported
as candidate metadata for subsequent rule selection.
"""
...
OUTPUT_ROOT = BASE_DIR / "data/output/pilot/sa/rule_based/rule_discovery"

RULE_START_RE = re.compile(r"^(header|body|rawbody|full|uri)\s+([A-Z0-9_]+)\s+(.*)$")
...
def _run_in_container(command: str) -> str:
    """
    Execute a shell command inside the SpamAssassin container.
...
def _find_rule_files() -> list[str]:
    """
    Locate SpamAssassin rule and plugin configuration files.

    Returns:
        list[str]: Paths to discovered .cf and .pre files inside the container.
    """
...
def _read_rule_file(container_path: str) -> str:
    """
    Read a SpamAssassin rule file from the container.

    Args:
        container_path (str): Path to the rule file inside the container.

    Returns:
        str: Textual content of the rule file.
    """
...
def _classify_surface(rule_type: str, expression: str) -> str:
    """
    Classify the message surface primarily targeted by a rule.
    ...
    Returns:
        str: Subject, body, or other.
    """

    expr = expression.lower()

    if rule_type == "header":
        return "subject" if "subject" in expr else "other"
...
def _classify_breakability(rule_type: str, expression: str) -> str:
    """
    Estimate whether a rule may be affected by U+200B token disruption.
    ...
    Returns:
        str: Estimated breakability classification.
    """

    expr = expression.lower()

    # Use lexical and structural hints to heuristically estimate salting sensitivity.
    lexical_hints = [
        "subject", "body", "rawbody", r"\b", "[a-z", "[A-Z", "[a-zA-Z",
        "verify", "family", "seen", "account", "password", "click", "secure",
    ]
    structural_hints = [
        "all_caps", "charset", "mime", "base64", "html_", "message-id",
        "date", "from:", "to:", "received",
    ]
...
def _parse_rule_files(rule_files: list[str]) -> list[dict]:
    """
    Parse SpamAssassin rule files and collect relevant rule metadata.

    Args:
        rule_files (list[str]): Rule files discovered inside the container.

    Returns:
        list[dict]: Normalized candidate rule records.
    """
    ...
    for rule_name, data in sorted(rules.items()):
        ...
        # Keep only rule types relevant for visible-content pilot analysis.
        ...
        rows.append(
            {
                ...
                "breakability_u200b": _classify_breakability(rule_type, expression),
...
def _write_csv(path: Path, rows: list[dict]) -> None:
    """
    Write discovered rule records to a semicolon-delimited CSV file.
...
def run_sa_rule_discovery() -> None:
    """
    Discover, classify, and export SpamAssassin rule candidates.
    """
